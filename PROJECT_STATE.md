...
Последнее обновление: 07.08.2026 (сверено с git-историей до `0e8d51f`) Schema version: 8 (канон `facts.{nodes,items}` + регенерируемое зеркало) Версия: **beta.V1.1.4.2**
...
* Supabase: `deck_words` (462 слова, 15 колод, группы по 8, с транслитерацией) + `user_word_progress` (telegram_id, word_id, seen, correct, wrong, sm2, introduced). RLS off. Источник: курированные 500 слов (`WORD_CATEGORIES`), мусор отфильтрован.
...
Открытые баги - в `BUGLOG.md` (отдельный лог, новое сверху). 07.08.2026 разобраны и закрыты BUG-001 (M1.4, перевод גדולה), BUG-003 (G1.1, сноска про שׁוֹתֶה/ל"ה), BUG-004 (C2, позиция «?» в RTL-строках), BUG-005 (M2.1, `answer ∈ options` — плюс разовый прогон линта по всем 393 В4-итемам, нарушений больше нет), BUG-006 (M2.5, «домой» объяснено через исключение הַבַּיְתָה). Открыт остался только BUG-002 (нискуд нечитаем на телефоне — UX/визуальный, не тронут, нужна проверка на устройстве). Build + смоук (200/200) зелёные после всех правок.
