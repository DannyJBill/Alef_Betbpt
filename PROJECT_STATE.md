...
Последнее обновление: 09.08.2026 (v1.2 часть 2 — редизайн Home: тропа курса, «Продолжить», подсказки, закрыт BUG-022; часть 1 — модули пути, подгруппы-чекпоинты, 10 узлов-экзаменов; см. `CHANGELOG.md`) Schema version: 8 (канон `facts.{nodes,items}` + регенерируемое зеркало) Версия: **beta.V1.1.4.2**
