...
Последнее обновление: 09.08.2026 (v1.2 часть 3 — единый экран словаря `VocabularyScreen` (слова + модули курса + темы + буквы), фикс BUG-023 (слова колод не переживали перезагрузку) + meta-снапшот в `facts`; часть 2 — редизайн Home: тропа курса, «Продолжить», подсказки, закрыт BUG-022; часть 1 — модули пути, подгруппы-чекпоинты, 10 узлов-экзаменов; см. `CHANGELOG.md`) Schema version: 8 (канон `facts.{nodes,items}` + регенерируемое зеркало) Версия: **beta.V1.1.4.2**

> ⚠️ v1.2 (части 1–2) — в рабочей копии, **не закоммичено** на момент этой записи (`git status`: изменены `App.jsx`/`StatsContext.jsx`/`curriculum.js`/`HomeScreen.jsx`/`StudyScreen.jsx`, новые `pathTheme.js`/`ExamScreen.jsx`/`public/mascot/`). Прод по-прежнему на `beta.V1.1.4.2`.
...
> 📌 Колоды `g_nouns`/`g_adjectives`/`g_verbs`/`conj`/`g_phrases` (10.08.2026, закоммичено `77a5665`) — слова внутри пересортированы по смысловым подгруппам (было по алфавиту), затем нарезаны на группы по 8 (по 4 — только `g_phrases`, было тоже 8); в базе (Supabase `deck_words`) поменялись только `chunk`/`ord`, состав колод не тронут. Покрытие смысловыми подгруппами частичное (33–91% по колоде в зависимости от размера/однородности, остаток — «Прочее» по алфавиту) — открытый словарь без тезауруса, подробности и метод — `CHANGELOG.md`. 15 тематических колод (`family`/`food`/...) не тронуты.
...
* **VocabularyScreen** (новый, v1.2 часть 3) — единый экран «Мой словарь» вместо `ReadingScreen(dictOnly)`: сводка (в словаре/знаю/слабых — кликабельный фильтр по статусу) + 🃏 Повторить всё / ✅ Проверить всё; переключатель группировки **Все слова** (плоский список+поиск) / **По урокам** (аккордеон по модулям `COURSE_PATH`, только изученное, `helpers/dictionary.js: groupByLessonChapter`) / **По темам** (встроенный `DecksScreen`, гейт `DECKS_UNLOCK_NODE`, «← Словарь» возвращает в переключатель — отдельной точки входа «Ещё слова» больше нет); персистентная карточка «🔤 Буквы» открывает `CardsScreen`; «✨ Ты уже можешь сказать» (`PhraseLocksSection`, экспортирован из `ReadingScreen.jsx`). Любая активная сессия (карточки/квиз) занимает экран целиком, переключатель прячется.
...
`tests/smoke_v7.mjs` — 201 проверка:
...
⚠️ 09.08.2026 (v1.2, при пуше): смоук показал 12 провалов — не регрессия, а рассинхрон с v1.2 частью 1: часть графовых проверок ждала СТАРЫЕ прямые цепочки (M1.4→C1, G1.6→SL1.1, M2.9→G2.1/C4, CH1.4→CH2.1, CH2.1→CH3.1, G2.4→G3.1, G3.5→C5.1, SL1.3→SL1.4), а после вставки EX-узлов гейты теперь идут через экзамен подгруппы (EX1.1→C1 и т.д. — см. `CHANGELOG.md`). Тестовые фикстуры поправлены под новые цепочки (не ослаблены — где было можно, добавлены доп. assert'ы на «заперто без EX» + «открыто после EX»), 201/201 зелёные.

`npm run build` собирается чисто (проверено 09.08.2026, 75 модулей, один Vite-варнинг про размер чанка >500 kB — не ошибка).
