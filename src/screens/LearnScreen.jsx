...
export default function LearnScreen() {
  ...
  const [phase, setPhase]       = useState('groups'); // groups|studying|test|result
  ...
  function buildQuestions(letters) {
    return shuffle(letters).map((letter, i) => {
      const type = i%2===0 ? 'letter-to-name' : 'name-to-letter';
      const wrong = shuffle(letters.filter(l=>l.id!==letter.id)).slice(0,3);
      if (type==='letter-to-name') {
        return { type, letter, prompt: letter.symbol, promptLabel:'Как называется эта буква?',
          correct: letter.name, options: shuffle([...wrong.map(l=>l.name), letter.name]) };
      } else {
        return { type, letter, prompt: letter.name, promptLabel:'Найди эту букву:',
          correct: letter.symbol, options: shuffle([...wrong.map(l=>l.symbol), letter.symbol]) };
      }
    });
  }
  ...
  function nextLetter() {
    if (letterIdx < groupLetters.length-1) setLetterIdx(i=>i+1);
      ...
      setQuestions(qs); setQIdx(0); setAnswered(null); setCorrect(0); setPhase('test');
    }
  }
  ...
  function handleAnswer(option) {
    ...
    timerRef.current = setTimeout(() => {
      if (qIdx < questions.length-1) {
        setQIdx(i=>i+1); setAnswered(null); setCorrect(nextCorrect);
      } else {
  ...
  if (phase==='test' && questions.length) {
    ...
    return (
      <div className="pb-20 px-4 pt-4 max-w-md mx-auto flex flex-col gap-4">
        ...
        <div className={`rounded-3xl flex items-center justify-center ${dark?'bg-gray-800':'bg-gray-50'}`} style={{height:160}}>
          <span style={{
            fontSize: q.type==='letter-to-name'?110:32,
            fontFamily:'serif', fontWeight:'bold',
          }} className={dark?'text-white':'text-gray-900'}>{q.prompt}</span>
        </div>
      ...
      <div className="pb-20 px-4 pt-10 max-w-md mx-auto flex flex-col items-center gap-5 text-center">
        ...
        {isPassed && nextGrp && (
          <div className={`w-full rounded-2xl p-4 border ${GROUP_COLORS[nextGrp.color].border} ${GROUP_COLORS[nextGrp.color].bg}`}>
            ...
            <p className={`text-xs mt-0.5 ${GROUP_COLORS[nextGrp.color].text} opacity-80`}>{nextGrp.description}</p>
          </div>
        )}
        {isPassed ? (
